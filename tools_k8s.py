...
def _validate_integer(value: Any, min_val: Optional[int] = None, max_val: Optional[int] = None) -> Optional[str]:
    """Validate integer with optional min/max bounds and strict type checking"""
    if isinstance(value, str):
        if not re.match(r'^-?\d+$', value.strip()):
            return "Value must be a numeric integer"
...
K8S_ENV_NAME_REGEX = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")
K8S_CPU_REGEX = re.compile(r"^[0-9]+(\.[0-9]+)?m?$")
K8S_MEM_REGEX = re.compile(r"^[0-9]+(\.[0-9]+)?(Mi|Gi|Ti|Pi|Ei|Ki|M|G|T|P|E|K)?$")

def _validate_k8s_name(name: str) -> bool:
    if not name or len(name) > 253: return False
    return bool(K8S_NAME_REGEX.match(name)) and not any(c in name for c in " ;|&><`$")

def _validate_env_name(name: str) -> bool:
    return bool(name) and bool(K8S_ENV_NAME_REGEX.match(name))

def _validate_cpu_quantity(val: str) -> bool:
    if not isinstance(val, str) or not val or len(val) > 32: return False
    return bool(K8S_CPU_REGEX.match(val)) and not any(c in val for c in " ;|&><`$")

def _validate_memory_quantity(val: str) -> bool:
    if not isinstance(val, str) or not val or len(val) > 32: return False
    return bool(K8S_MEM_REGEX.match(val)) and not any(c in val for c in " ;|&><`$")

def _resolve_container(deployment: str, namespace: str, requested: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    """Returns (container_name, error_message). Enforces unique container resolution."""
    if not _validate_k8s_name(deployment) or not _validate_k8s_name(namespace):
        return None, "Invalid k8s resource names."
    if requested:
        return (requested, None) if _validate_k8s_name(requested) else (None, "Invalid requested container name.")
    success, stdout, stderr = SafeSubprocess.run(["kubectl", "get", "deployment", deployment, "-n", namespace, "-o", "json"])
    if not success: return None, f"Discovery failed: {stderr}"
    try:
        containers = json.loads(stdout).get("spec", {}).get("template", {}).get("spec", {}).get("containers", [])
        valid_names = [c["name"] for c in containers if _validate_k8s_name(c.get("name"))]
        if not valid_names: return None, "No valid containers found."
        if len(valid_names) == 1: return valid_names[0], None
        return None, f"Multiple containers found {valid_names}. Specify 'container' explicitly."
    except: return None, "JSON Parse Error"

# 2. Tool Implementations
def k8s_get_pod_logs(pod_name: str, namespace: str = "default", tail: int = 100) -> dict:
    if not _validate_k8s_name(pod_name) or not _validate_k8s_name(namespace):
        return standardize_response("error", error="Invalid k8s resource names.")
    if not isinstance(tail, int) or not (1 <= tail <= 1000):
        return standardize_response("error", error="Tail must be int between 1 and 1000.")
    success, stdout, stderr = SafeSubprocess.run(["kubectl", "logs", pod_name, "-n", namespace, f"--tail={tail}"])
    return standardize_response("ok" if success else "error", data={"logs": stdout}, error=stderr)

def k8s_get_pod_status(pod_name: str, namespace: str = "default") -> dict:
    if not _validate_k8s_name(pod_name) or not _validate_k8s_name(namespace):
        return standardize_response("error", error="Invalid k8s resource names.")
    success, stdout, stderr = SafeSubprocess.run(["kubectl", "get", "pod", pod_name, "-n", namespace, "-o", "json"])
    if success:
        try: return standardize_response("ok", data=json.loads(stdout))
        except: return standardize_response("error", error="Failed to parse k8s response.")
    return standardize_response("error", error=stderr)

def k8s_describe_pod(pod_name: str, namespace: str = "default") -> dict:
    if not _validate_k8s_name(pod_name) or not _validate_k8s_name(namespace):
        return standardize_response("error", error="Invalid k8s resource names.")
    success, stdout, stderr = SafeSubprocess.run(["kubectl", "describe", "pod", pod_name, "-n", namespace])
    return standardize_response("ok" if success else "error", data={"description": stdout}, error=stderr)

def k8s_rollout_restart(deployment: str, namespace: str = "default") -> dict:
    if not _validate_k8s_name(deployment) or not _validate_k8s_name(namespace):
        return standardize_response("error", error="Invalid k8s resource names.")
    success, stdout, stderr = SafeSubprocess.run(["kubectl", "rollout", "restart", f"deployment/{deployment}", "-n", namespace])
    return standardize_response("ok" if success else "error", data={"output": stdout.strip()}, error=stderr)

def k8s_patch_deployment_env(deployment: str, env_name: str, env_value: str, container: Optional[str] = None, namespace: str = "default") -> dict:
    if not isinstance(env_value, str) or len(env_value) > 4096: return standardize_response("error", error="Invalid env_value.")
    c_name, err = _resolve_container(deployment, namespace, container)
    if err: return standardize_response("error", error=err)
    if not _validate_env_name(env_name): return standardize_response("error", error="Invalid env_name.")
    patch = {"spec": {"template": {"spec": {"containers": [{"name": c_name, "env": [{"name": env_name, "value": env_value}]}]}}}}
    success, stdout, stderr = SafeSubprocess.run(["kubectl", "patch", "deployment", deployment, "-n", namespace, "--type", "strategic", "-p", json.dumps(patch)])
    return standardize_response("ok" if success else "error", data={"output": stdout.strip()} if success else None, error="Patch failed" if not success else None)

def k8s_patch_resource_limits(deployment: str, cpu_request: str = None, cpu_limit: str = None, memory_request: str = None, memory_limit: str = None, container: str = None, namespace: str = "default") -> dict:
    if not any([cpu_request, cpu_limit, memory_request, memory_limit]): 
        return standardize_response("error", error="No valid resource values specified.")
    
    c_name, err = _resolve_container(deployment, namespace, container)
    if err: return standardize_response("error", error=err)
    res = {}
    if cpu_request:
        if not _validate_cpu_quantity(cpu_request): return standardize_response("error", error="Invalid cpu_request.")
        res.setdefault("requests", {})["cpu"] = cpu_request
    if cpu_limit:
        if not _validate_cpu_quantity(cpu_limit): return standardize_response("error", error="Invalid cpu_limit.")
        res.setdefault("limits", {})["cpu"] = cpu_limit
    if memory_request:
        if not _validate_memory_quantity(memory_request): return standardize_response("error", error="Invalid memory_request.")
        res.setdefault("requests", {})["memory"] = memory_request
    if memory_limit:
        if not _validate_memory_quantity(memory_limit): return standardize_response("error", error="Invalid memory_limit.")
        res.setdefault("limits", {})["memory"] = memory_limit
    
    patch = {"spec": {"template": {"spec": {"containers": [{"name": c_name, "resources": res}]}}}}
    success, stdout, stderr = SafeSubprocess.run(["kubectl", "patch", "deployment", deployment, "-n", namespace, "--type", "strategic", "-p", json.dumps(patch)])
    return standardize_response("ok" if success else "error", data={"output": stdout.strip()} if success else None, error=stderr)
    ...
    try:
        ...
        try:
            ...
            if failed_init:
                ...
                try:
                    logs_excerpt = subprocess.check_output(
                        ["kubectl", "logs", pod_name, "-n", namespace, "-c", init_name, "--tail=50"],
                        text=True,
                        stderr=subprocess.STDOUT,
                    )
                except Exception:
        ...
        if not patched and owner_kind == "DaemonSet" and owner_name:
            ...
            if not patched:
                ...
                if skip_fallback_actions:
                    ...
                    subprocess.run(["kubectl", "delete", "pod", pod_name, "-n", namespace, "--wait=false"], check=True)
...
def get_pod_status(namespace: str = "default") -> dict:
    ...
    try:
        ...
        for item in pods_data.get("items", []):
            ...
            is_ready = pod_info["ready"]
            restarts = pod_info["restarts"]
            if phase == "Running" and is_ready and restarts == 0 and not pod_info["issues"]:
                healthy_count += 1
            else:
                unhealthy_count += 1
                # If not ready but running, add "Unready" issue
                if phase == "Running" and not is_ready and not pod_info["issues"]:
                    pod_info["issues"].append("Unready")
                
                # If has restarts but no other issues
                if restarts > 0 and not pod_info["issues"]:
                    pod_info["issues"].append(f"Restarts({restarts})")
                
                if pod_info["issues"] or phase != "Running" or restarts > 0:
...
def _validate_container_image(image: str) -> bool:
    """Validate container image format to prevent shell injection and ensure sanity."""
    if not isinstance(image, str) or not image or len(image) > 255:
        return False
    # No spaces or shell metacharacters allowed. Allowed: [a-zA-Z0-9._\-/:@]
    return bool(re.match(r'^[a-zA-Z0-9._\-/:@]+$', image))

def _validate_probe_path(path: str) -> bool:
    """Validate probe path for safety and k8s compliance."""
    if not isinstance(path, str) or not path.startswith("/") or len(path) > 256:
        return False
    return bool(re.match(r'^[a-zA-Z0-9/_.\-~]+$', path))

def k8s_patch_deployment_image(deployment: str, container: str, image: str, namespace: str = "default") -> dict:
    """Updates the image of a specific container in a deployment."""
    if not _validate_k8s_name(deployment) or not _validate_k8s_name(namespace):
        return standardize_response("error", error="Invalid k8s resource names.")
    if not _validate_container_image(image):
        return standardize_response("error", error="Invalid container image format.")
    
    c_name, err = _resolve_container(deployment, namespace, container)
    if err: return standardize_response("error", error=err)

    success, stdout, stderr = SafeSubprocess.run([
        "kubectl", "set", "image", f"deployment/{deployment}", 
        f"{c_name}={image}", "-n", namespace
    ])
    return standardize_response("ok" if success else "error", data={"output": stdout.strip()}, error=stderr)

def k8s_patch_probe(deployment: str, container: str, probe_type: str, 
                    path: str = None, port: int = None, 
                    initial_delay_seconds: int = None, 
                    period_seconds: int = None, 
                    failure_threshold: int = None,
                    namespace: str = "default") -> dict:
    """Patches liveness or readiness probe configuration for a container."""
    if probe_type not in ["readinessProbe", "livenessProbe"]:
        return standardize_response("error", error="probe_type must be exactly readinessProbe or livenessProbe.")
    
    if not any([path, port, initial_delay_seconds is not None, period_seconds is not None, failure_threshold is not None]):
        return standardize_response("error", error="No probe fields specified.")
    ...
    probe_config = {}
    if path:
        if not _validate_probe_path(path): return standardize_response("error", error="Invalid probe path.")
        probe_config.setdefault("httpGet", {})["path"] = path
    if port is not None:
        err = _validate_integer(port, 1, 65535)
        if err: return standardize_response("error", error=f"Invalid port: {err}")
        probe_config.setdefault("httpGet", {})["port"] = int(port)
    
    if initial_delay_seconds is not None:
        err = _validate_integer(initial_delay_seconds, 0, 3600)
        if err: return standardize_response("error", error=f"Invalid initial_delay_seconds: {err}")
        probe_config["initialDelaySeconds"] = int(initial_delay_seconds)
        
    if period_seconds is not None:
        err = _validate_integer(period_seconds, 1, 3600)
        if err: return standardize_response("error", error=f"Invalid period_seconds: {err}")
        probe_config["periodSeconds"] = int(period_seconds)
        
    if failure_threshold is not None:
        err = _validate_integer(failure_threshold, 1, 100)
        if err: return standardize_response("error", error=f"Invalid failure_threshold: {err}")
        probe_config["failureThreshold"] = int(failure_threshold)

    patch = {"spec": {"template": {"spec": {"containers": [{"name": c_name, probe_type: probe_config}]}}}}
...
def k8s_rollout_undo(deployment: str, namespace: str = "default", revision: int = None) -> dict:
    ...
    if revision is not None:
        err = _validate_integer(revision, 1, 100000)
        if err: return standardize_response("error", error=f"Invalid revision: {err}")
        cmd.append(f"--to-revision={int(revision)}")
    
    success, stdout, stderr = SafeSubprocess.run(cmd)
    return standardize_response("ok" if success else "error", data={"output": stdout.strip()}, error=stderr)
