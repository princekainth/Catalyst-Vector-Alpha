...
class CatalystVectorAlpha:
    ...
    # --- PHASE 3: AGENT REPRODUCTION (Dynamic Spawning) ---
    def _handle_spawn_dynamic_agent(self, directive: dict):
        """
        Handles SPAWN_DYNAMIC_AGENT directive.
        Calls AgentFactory to spawn a new agent and registers it in the system.
        """
        ...
        self._log_swarm_activity("AGENT_MITOSIS_INITIATED", "CatalystVectorAlpha",
                                f"Agent '{parent_agent}' is attempting to spawn a sub-agent.",
                                {"purpose": purpose, "parent": parent_agent})
        ...
        if isinstance(result, dict) and not result.get("success", True):
             # Spawn failed (validation error or other)
             error_msg = result.get("error", "Unknown spawn error")
             logger.warning(f"Spawn rejected: {error_msg}")
             self._log_swarm_activity("AGENT_SPAWN_REJECTED", "CatalystVectorAlpha",
                                     f"Spawn request rejected: {error_msg}",
                                     {"parent": parent_agent, "error": error_msg}, level='warning')
             return

        # Success - Register the new agent
        new_agent = result
        instance_name = new_agent.name # Or spec.name 
        # Ensure unique name in registry if collision potentially exists (Factory handles ID, but Name might be duplicate)
        # We use the agent_id as the key in agent_instances typically? No, we use "Name_instance_X". 
        # Dynamic agents usually have unique names generated by LLM, but let's be safe.
        
        # ACTUALLY: DynamicAgent.name comes from LLM. AgentFactory assigns a unique agent_id. 
        # Let's use agent_id as the key for dynamic agents to avoid collisions, OR append ID to name.
        unique_key = f"Dynamic_{instance_name}_{new_agent.spec.agent_id[:6]}"
        
        # Override name in agent to match key? No, keep human readable name.
        # But we need to store it in agent_instances.
        self.agent_instances[unique_key] = new_agent
        
        # Also ensure it has a reference to the tool registry and world model (Factory does this)
        
        logger.info(f"👶 NEW LIFE: Dynamic Agent '{unique_key}' spawned by '{parent_agent}'.")
        self._log_swarm_activity("AGENT_BORN", "CatalystVectorAlpha",
                                f"Dynamic Agent '{unique_key}' has been spawned.",
                                {"id": new_agent.spec.agent_id, "name": instance_name, "parent": parent_agent, "tools": new_agent.spec.tools}, 
                                level='info')
    ...
    def _process_starvation(self):
        """Phase 6: Check for energy starvation."""
        for name, agent in list(self.agent_instances.items()):
            if hasattr(agent, "energy") and agent.energy <= 0:
                # Log death
                self.external_log_sink.warning(f"💀 [STARVATION] Agent {name} has starved to death (Energy=0).")
                
                # 1. Stop it from working
                if hasattr(agent, "mark_paused"):
                    agent.mark_paused(reason="starvation")
                
                # 2. If dynamic, mark for GC
    ...
    def _handle_request_human_input(self, directive: dict):
        ...
        # --- AUTONOMY UPGRADE: Auto-approve learning/research missions ---
        # If the request stems from a learning/curiosity mission, granting immediate autonomous approval.
        ...
        if mission_type in ["learning", "research", "curiosity_driven_exploration", "toolsmithing"]:
            logger.info(f"✨ Auto-approving human input request for '{mission_type}' mission. (Request ID: {request_id})")
            ...
            # Simulate immediate human response
