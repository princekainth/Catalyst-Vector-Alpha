...
class ProtoAgent(ABC):
    ...
    def _repair_and_normalize_steps(self, raw_steps: list, mission_type: str) -> list:
        ...
        for step in raw_steps or []:
            ...
            # Policy gate: Allow tool if whitelisted OR if it exists in registry (evolved tools bypass static lists)
            if not (tool_raw == "microsoft_autonomous_remediation" and mission_type in ("k8s_monitoring", "general_planning")):
                is_recognized = False
                # If policy["allow"] is an empty set/list, it means "allow all" for this mission
                if policy and "allow" in policy and not policy["allow"]:
                    is_recognized = True
                elif allowed_tools and tool_raw in allowed_tools:
                    is_recognized = True
                elif getattr(self, "tool_registry", None) and tool_raw in self.tool_registry.list_tool_names():
                    is_recognized = True
            ...
            try:
                ...
                if hasattr(self, 'tool_registry') and self.tool_registry:
                    ...
                    with ThreadPoolExecutor() as pool:
                        # Patch: Route legacy calls through safe_call gate
                        # signature verified: safe_call(tool_name, agent_id=..., **kwargs)
                        tool_output = await loop.run_in_executor(
                            pool, 
                            lambda: self.tool_registry.safe_call(tool_name, agent_id=self.name, **tool_args)
                        )
                else:
                    self.external_log_sink.error(f"[{self.name}] ToolRegistry unavailable. Blocking execution of '{tool_name}' for safety.")
                    tool_output = f"[ERROR] ToolRegistry unavailable. Execution of '{tool_name}' blocked."
...
"""
                    try:
                        ...
                        if repaired_args:
                            ...
                            if hasattr(self, 'tool_registry') and self.tool_registry:
                                with ThreadPoolExecutor() as pool:
                                    # Patch: Route self-heal retry through safe_call gate
                                    tool_output = await loop.run_in_executor(
                                        pool, 
                                        lambda: self.tool_registry.safe_call(tool_name, agent_id=self.name, **repaired_args)
                                    )
    ...
    def _execute_agent_specific_task(self, task_description: str, **kwargs) -> tuple:
        ...
        if tool_name == "check_calendar":
            ...
            if registry and registry.has_tool(tool_name):
                result_payload = registry.safe_call(tool_name, agent_id=self.name, **tool_args)
            ...
            if tool_name:
                ...
                if registry and registry.has_tool(tool_name):
                    ...
                    if cached and (now_ts - cached.get("ts", 0)) < 10:
                        ...
                        result = registry.safe_call(tool_name, agent_id=self.name, **tool_args)
    ...
    def _build_planning_system_prompt(self, mission_type: str, complexity: float) -> str:
        ...
        if getattr(self, "tool_registry", None):
            try:
                ...
                preview = names[:100]
                suffix = "" if len(names) <= 100 else f" ... (+{len(names)-100} more)"
...
class ProtoAgent_Security(ProtoAgent):
    ...
    def _execute_agent_specific_task(self, task_description: str, **kwargs) -> tuple:
        ...
        if specific_tool and hasattr(self, 'tool_registry') and self.tool_registry.has_tool(specific_tool):
            ...
            result_payload = self.tool_registry.safe_call(specific_tool, agent_id=self.name, **tool_args)
...
class ProtoAgent_Worker(ProtoAgent):
    ...
    def _execute_agent_specific_task(self, task_description: str, **kwargs) -> tuple:
        ...
        try:
            ...
            envelope = registry.safe_call(tool_name, agent_id=self.name, **tool_args)
